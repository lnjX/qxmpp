...
void tst_QXmppStream::testProcessData()
{
    ...
    connect(&stream, &QXmppLoggable::logMessage, this, [](QXmppLogger::MessageType type, const QString &msg) {
        qDebug() << type << "LOG" << msg;
    });
    ...
    // test partial data
    stream.processData(R"(<message from="juliet@im.example.com" to="stpeter@im.example.com">)");
    QCOMPARE(onStreamReceived.size(), 1);
    QCOMPARE(onStanzaReceived.size(), 1);
    QCOMPARE(onStarted.size(), 0);
    ...
    QCOMPARE(features.tagName(), QStringLiteral("message"));
    QCOMPARE(features.namespaceURI(), QStringLiteral("jabber:client"));
